...
dgp_function = function(x){
    # x[1]^2 * cos(x[1]) + x[2]^2 * cos(x[2]) + x[3]^2 * cos(x[3])
    
    # term1 <- 0.75 * exp(-(9*x[1]-2)^2/4 - (9*x[2]-2)^2/4)
    # term2 <- 0.75 * exp(-(9*x[1]+1)^2/49 - (9*x[2]+1)/10)
    # term3 <- 0.5 * exp(-(9*x[1]-7)^2/4 - (9*x[2]-3)^2/4)
...
run_sim <- function(i, n, c_seq, X_test_random, mu_rand, mu_fixed, draw_random_data, est_variance=F, verbose=F){
    ...
    dnn_rand_estimate <- tdnn:::tune_dnn_no_dist_thread(X,Y,X_test_random,
                                                        s_seq = dnn_s_seq,
                                                        W0_ = W_0,
                                                        estimate_variance=T,num_threads = num_threads)
    if(verbose) tictoc::toc()
...
file_path <- glue::glue("setting_2_p_{p}_{data_type}_data_{draw_random_data}_tuned_de_dnn_{str_replace_all(strftime(Sys.time(), '%Y-%m-%d_%H_%M'),'-', '_')}.rds")
results_data %>% 
    write_rds(.,
              file_path
    )
...
print(file_path)
print(glue::glue("scp -i ~/Dropbox/Spring\ 2022/MISC/tdnn.pem ubuntu@{Sys.getenv('PUB_IP')}:{file_path} /Users/patrick/tdnn/non_parametric_sims/setting_2/results_tables"))
# }
